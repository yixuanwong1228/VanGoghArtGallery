...
import JEEFACEFILTERAPI from './jeelizFaceFilter.moduleES6.js';
//import JEEFACEFILTERAPI from './jeelizFaceFilter.js';
import NNC_JSON from '/assets/js/FaceChangeGame/NN_VERYLIGHT_1.json' with{type:'json'};

//import image_picture_frame from '/assets/img/FaceChangeGame/picture_frame.png';
...
export default class VanGoghFaceFilter {
    ...
    constructor(
        ...
        this.picture_frame_image.src = '/assets/img/FaceChangeGame/picture_frame.png';
    }
    ...
    saveImage(_event) {
        ...
        // Compute topPx and leftPx in the artpainting canvas image ref:
        let topPx = ARTPAINTING.image.height * ARTPAINTING.positionFace[1];
        let leftPx = ARTPAINTING.image.width * ARTPAINTING.positionFace[0];
        let widthFacePx = ARTPAINTING.image.width * ARTPAINTING.scaleFace[0];
        let heightFacePx = ARTPAINTING.image.height * ARTPAINTING.scaleFace[1];
        ...
        // Draw the face first (without inversion)
        ctx.drawImage(
        ...
        // Invert the context vertically to draw the painting
        ctx.save(); // Save the current state
        ctx.scale(1, -1); // Flip vertically
        ctx.drawImage(document.getElementsByClassName('artPainting')[0], 150, -final_canvas.height + 150);
        ...
        temp_canvas.width = final_canvas.width;
        temp_canvas.height = final_canvas.height;
        let temp_ctx = temp_canvas.getContext("2d");
    
        // Copy the current state to the temporary canvas
        temp_ctx.drawImage(final_canvas, 0, 0);
    
        // Clear the final canvas and invert it back
        ctx.clearRect(0, 0, final_canvas.width, final_canvas.height);
        ctx.save(); // Save the current state
        ctx.scale(1, -1); // Flip vertically
        ctx.drawImage(temp_canvas, 0, -final_canvas.height);
        ctx.restore(); // Restore the original state
